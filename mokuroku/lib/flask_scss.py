...
class Scss(object):
	...
	def __init__(self, app, static_dir=None, asset_dir=None, load_paths=None, namespace=Namespace()):
		'''
		...
		load_path_list = ([self.asset_dir] if self.asset_dir else []) + (load_paths or app.config.get('SCSS_LOAD_PATHS', []))

		# pyScss.log = app.logger
		self.compiler = Compiler(search_path=load_path_list, namespace=namespace)
		if self.app.testing or self.app.debug:
			self.set_hooks()

	def set_asset_dir(self, asset_dir):
		asset_dir = asset_dir or self.app.config.get('SCSS_ASSET_DIR', None) or op.join(self.app.root_path, 'assets')
	...
	def set_static_dir(self, static_dir):
		static_dir = static_dir or self.app.config.get('SCSS_STATIC_DIR', None) or op.join(self.app.root_path, self.app.static_folder)
		...
		for folder, _, files in os.walk(self.asset_dir):
			for filename in fnmatch.filter(files, '*.scss'):
				...
				if filename.startswith('_') and src_path not in self.partials:
					...
					dest_path = src_path.replace(self.asset_dir, self.static_dir).replace('.scss', '.css')
	...
	def update_scss(self):
		...
		if self.partials_have_changed():
			...
			return
		for asset, dest_path in self.assets.items():
			dest_mtime = op.getmtime(dest_path) if op.exists(dest_path) else -1
